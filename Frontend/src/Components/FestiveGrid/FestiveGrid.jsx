...
const SAMPLE_PRODUCTS = [
  {
    ...
    inStock: true,
    description:
      "Hand-finished decorative idol with premium detailing. Perfect for gifting and home altars.",
  },
  {
    id: 2,
...
const FestiveGrid = () => {
  ...
  const allPurposes = useMemo(
    () => unique(SAMPLE_PRODUCTS.map((p) => p.purpose)),
    []
  );
  const allMaterials = useMemo(
    () => unique(SAMPLE_PRODUCTS.map((p) => p.material)),
    []
  );
  ...
  const PRICE_MIN = useMemo(
    () => Math.min(...SAMPLE_PRODUCTS.map((p) => p.price)),
    []
  );
  const PRICE_MAX = useMemo(
    () => Math.max(...SAMPLE_PRODUCTS.map((p) => p.price)),
    []
  );
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`festivegrid-backdrop ${drawerOpen ? "open" : ""}`}
        onClick={closeDrawer}
        aria-hidden={!drawerOpen}
      />

      <div
        className={`festivegrid-container ${drawerOpen ? "drawer-open" : ""}`}
        ref={containerRef}
        aria-live="polite"
      >
        {/* Sidebar (drawer on mobile) */}
        <aside
          className={`festivegrid-filters ${drawerOpen ? "open" : ""}`}
          aria-label="Filters"
          role="region"
        >
          <div className="festivegrid-drawer-header">
            <button
              className="festivegrid-drawer-close"
              onClick={closeDrawer}
            ...
            {allPurposes.map((p) => (
              <label key={p} className="festivegrid-checkbox">
                <input
                  ...
                  onChange={() => toggle(p, selectedPurposes, setSelectedPurposes)}
                />
                <span>
                  {p} <small className="festivegrid-count">(62)</small>
                </span>
              </label>
            ))}
          </div>
            ...
            {allSizes.map((s) => (
              <label key={s} className="festivegrid-checkbox">
                <input
                  ...
                  onChange={() => toggle(s, selectedSizes, setSelectedSizes)}
                />
                <span>{s} inches</span>
              </label>
            ))}
          ...
          <div className="festivegrid-filter-group">
            <h4>Availability</h4>
            <label className="festivegrid-checkbox">
              <input
                type="checkbox"
                checked={selectedAvail.includes("in")}
                onChange={() => toggle("in", selectedAvail, setSelectedAvail)}
              />{" "}
              <span>In stock</span>
            </label>
            <label className="festivegrid-checkbox">
              <input
                type="checkbox"
                checked={selectedAvail.includes("out")}
                onChange={() => toggle("out", selectedAvail, setSelectedAvail)}
              />{" "}
          ...
          <div className="festivegrid-filter-group festivegrid-price-area">
            <div className="festivegrid-price-header">
              <h4>Price</h4>
              <button className="festivegrid-reset-btn" onClick={resetAll}>
                Reset
              </button>
            </div>

            <div
              className="festivegrid-range-wrap"
              style={{ position: "relative", height: 36 }}
            >
              <input
                aria-label="Minimum price"
                className="festivegrid-range-input"
                type="range"
                min={PRICE_MIN}
                max={PRICE_MAX}
                value={minVal}
        ...
        {/* Main */}
        <main className="festivegrid-main" role="main">
          <div className="festivegrid-topbar" role="toolbar" aria-label="Options">
            <div className="festivegrid-top-left">
              <button
                className="festivegrid-filter-toggle"
            ...
            <div className="festivegrid-top-right">
              <div
                className="festivegrid-view-area"
                role="tablist"
                aria-label="View toggle"
              >
                <button
                  className={`festivegrid-view-btn ${view === "list" ? "active" : ""}`}
                ...
                <button
                  className={`festivegrid-view-btn ${view === "grid" ? "active" : ""}`}
          ...
          <div
            className={`festivegrid-products ${
              view === "grid" ? "festivegrid-mode-grid" : "festivegrid-mode-list"
            }`}
          >
            {/* GRID MODE: show flipped cards with Details */}
            {view === "grid" && (
              <div className="festivegrid-grid" role="list">
                {filtered.map((p) => (
                  <article
                    key={p.id}
                    className="festivegrid-card"
                    tabIndex={0}
                    aria-labelledby={`p-${p.id}-title`}
                  >
                    <div
                      className={`festivegrid-card-inner ${flipped[p.id] ? "flipped" : ""}`}
                    >
                      {/* FRONT */}
                      <div
                        className="festivegrid-face festivegrid-front"
                        aria-hidden={!!flipped[p.id]}
                      >
                          ...
                          <div className="festivegrid-imgwrap">
                            ...
                            <div className="festivegrid-hover-actions">
                              ...
                              <button
                                ...
                                onClick={() => alert(`Quick added "${p.title}" to cart.`)}
                              >
                                <Icon name="cart" />
                              </button>
                            </div>
                        ...
                        <div className="festivegrid-info">
                          ...
                          <div className="festivegrid-meta">
                            <div className="festivegrid-rating">
                              <Icon name="star" />
                              <span className="festivegrid-rating-num">
                                {p.rating.toFixed(1)}
                              </span>
                          ...
                          <div className="festivegrid-actions">
                            <button
                              className="festivegrid-ghost-btn"
                              onClick={() => toggleFlip(p.id)}
                              aria-expanded={!!flipped[p.id]}
                            >
                              Details
                            </button>

                            <button
                              className="festivegrid-primary-btn"
                              onClick={() => alert(`Quick added "${p.title}" to cart.`)}
                            >
                      ...
                      {/* BACK (flipped) */}
                      <div
                        className="festivegrid-face festivegrid-back"
                        aria-hidden={!flipped[p.id]}
                      >
            ...
            {/* LIST MODE: compact rows — no flip/details */}
            {view === "list" && (
              <div className="festivegrid-list" role="list">
                {filtered.map((p) => (
                  <article
                    key={p.id}
                  ...
                  >
                    ...
                    <div className="festivegrid-product-body">
                      <h3 className="festivegrid-product-title" id={`p-${p.id}-title`}>
                        {p.title}
                      </h3>
                      ...
                      <div className="festivegrid-product-meta">
                        <div className="festivegrid-rating">
                          ...
                          <span className="festivegrid-reviews">
                            {p.reviews} reviews
                          </span>
                        </div>
                      ...
                      <p className="festivegrid-desc" aria-hidden="false">
                        {p.description}
                      </p>
                      ...
                      <div className="festivegrid-product-ctas">
                        {p.inStock ? (
                          <button
                            className="festivegrid-btn-quick"
                            onClick={() => alert(`Quick added "${p.title}" to cart.`)}
                          >
                            QUICK ADD
                          </button>
                          ...
                          <button
                            className="festivegrid-btn-select"
                            onClick={() => alert(`Select options for "${p.title}".`)}
                          >
                            SELECT OPTIONS
                          </button>
                        )}
